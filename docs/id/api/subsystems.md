# Referensi Subistem & Konstanta

`leaves-guardian` memiliki arsitektur modular yang tersusun dari mesin keandalan terarah, utilitas developer, dan pengendali infrastruktur. Dokumen ini berfungsi sebagai peta referensi API sentral yang memetakan jalur akses subistem publik pada `LeavesClient`, konstruktor utilitas mandiri, fungsi pembantu (helpers), serta katalog lengkap tanpa pemotongan (`...`) dari seluruh konstanta publik, enum, status lifecycle, dan opsi konfigurasi default.

---

## 1. Peta Akses Subistem & Arsitektur

`leaves-guardian` mengorganisir komponen internal dan eksternal ke dalam tiga klasifikasi fungsional:
...
### Tabel Properti Akses Langsung pada LeavesClient

Tabel berikut merangkum seluruh pengakses subistem publik yang terpasang langsung pada instance `LeavesClient`:

| Properti pada `client` | Instance Kelas | Fungsi Utama | Method Publik Utama |
| :--- | :--- | :--- | :--- |
| `client.terminal` | `LeavesTerminal` | Logging terminal semantik & output konsol sadar privasi. | `info()`, `success()`, `warn()`, `error()`, `debug()`, `mask()` |
| `client.store` | `SmartStore` | Penyimpanan state JSON & key-value in-memory atomik dengan TTL. | `get()`, `set()`, `delete()`, `has()`, `keys()`, `clear()`, `sweep()`, `namespace()` |
| `client.recovery` | `SessionRecovery` | Verifikasi integritas sesi otomatis & pemulihan snapshot disk. | `checkSessionHealth()`, `createSnapshot()`, `restoreLatestValidSnapshot()`, `quarantineCurrentSession()`, `cleanEligibleAuthFiles()`, `clearBackups()` |
| `client.health` | `HealthMonitor` | Diagnostik observabilitas, lag event loop, dan pemantauan memori. | `start()`, `stop()`, `pause()`, `resume()`, `getStatus()`, `getMetrics()`, `getHistory()`, `registerProbe()` |
| `client.watchdog` | `Watchdog` | Verifikasi keaktifan socket & terminasi koneksi zombie. | `start()`, `stop()`, `pause()`, `resume()`, `ping()`, `recordActivity()`, `getStatus()` |
| `client.memoryGuard` | `MemoryGuard` | Observasi memori proses & mitigasi heap bertahap. | `start()`, `stop()`, `pause()`, `resume()`, `getStatus()`, `getMetrics()`, `getHistory()`, `registerMitigationHook()` |
| `client.traffic` | `TrafficController` | Antrean keluar prioritas, keadilan FIFO, backpressure & pacing. | `enqueue()`, `cancelTask()`, `pause()`, `resume()`, `drain()`, `getStats()`, `getQueueStatus()`, `getHistory()` |
| `client.media` | `MediaPipeline` | Preparasi media, proteksi SSRF, validasi MIME & spill file sementara. | `prepare()`, `abortJob()`, `abortAllJobs()`, `getStats()`, `cleanupTempDir()` |
| `client.rateLimiter` | `IngressRateLimiter` | Pembatasan laju pesan masuk dengan pelacak sliding window. | `consume()`, `check()`, `resetKey()`, `resetAll()`, `getStats()` |
| `client.deduplicator` | `IngressDeduplicator` | Deteksi pesan masuk duplikat & penekanan duplikasi via cache. | `isDuplicate()`, `register()`, `checkAndRegister()`, `clear()`, `getStats()` |
| `client.autoDelete` | `AutoDeleteManager` | Penghapusan pesan terjadwal dengan prinsip zero-rollback safety. | `register()`, `cancel()`, `cancelAll()`, `getTask()`, `getStats()` |

---

## 2. Utilitas On-Demand & Fungsi Pembantu (Helpers)

### 2.1 Kelas Utilitas Interaktif On-Demand

Kelas-kelas ini di-instantiate per alur interaksi dan beroperasi di atas objek pesan ternormalisasi (`Message`):
...
| Kelas | Signature Konstruktor | Deskripsi |
| :--- | :--- | :--- |
| `MessageCollector` | `new MessageCollector(client, options)` | Mengumpulkan pesan masuk yang memenuhi kriteria filter tertentu dalam batas waktu/jumlah terkonfigurasi. |
| `Prompt` | `new Prompt(client, options)` | Memandu pengguna melalui wizard interaktif tanya-jawab satu langkah maupun multi-langkah. |
| `Paginator` | `new Paginator(client, options)` | Merender daftar multi-halaman, katalog, atau menu dengan kontrol navigasi interaktif. |
| `EphemeralMessage` | `new EphemeralMessage(client, options)` | Mengonfigurasi dan mengirim pesan sementara yang hilang otomatis (disappearing messages). |

### 2.2 Fungsi Pembantu Standalone (Helpers)
...
| Fungsi | Signature | Tipe Kembalian | Deskripsi |
| :--- | :--- | :--- | :--- |
| `withAdReply` | `(contextInfo: object, options: object)` | `object` | Menyisipkan metadata pratinjau tautan interaktif / ad reply ke `contextInfo` Baileys. |
| `withChannelForward` | `(contextInfo: object, options: object)` | `object` | Menyisipkan metadata header terusan saluran resmi / newsletter ke `contextInfo`. |
| `resolveMedia` | `async (client: object, source: any, type: string, opts?: object)` | `Promise<any>` | Menyelesaikan buffer media, URL remote, atau path file lokal untuk pengiriman Baileys. |
| `generateStatCard` | `(stats: object, options?: object)` | `string` | Memformat widget ringkasan dashboard ASCII untuk penyajian metrik/kesehatan bot. |

---

## 3. Infrastruktur Runtime Internal

Subistem berikut di-instantiate dan dikelola secara internal oleh `LeavesClient`. Komponen ini tidak dimaksudkan untuk di-instantiate manual oleh konsumen, namun perilakunya dapat dikonfigurasi melalui opsi konstruktor `LeavesClient`:

- **`ConnectionManager`**: Mengelola pembuatan socket aktif Baileys, relai event, dan koordinasi kode QR/Pairing.
- **`SessionManager`**: Menangani persistensi state autentikasi multi-file, pembuatan folder sesi, dan validasi creds.
- **`ReconnectManager`**: Mengelola penjadwalan rekoneksi otomatis berbasis exponential backoff dengan jitter.
- **`EventManager`**: Mengoordinasikan langganan event emitter internal dan perutean event ternormalisasi.
- **`TerminalManager`**: Bus event sentral untuk logging diagnostik, multipleksing sink, dan sanitasi privasi.
- **`PresentationAdapter`**: Menjembatani event domain `LeavesClient` menjadi log presentasi terminal yang semantik.
- **`LoggerAdapter`**: Mengadaptasi stream logger standar Pino ke dalam `LeavesTerminal`.

---

## 4. Kamus Sentral Konstanta & Enum Publik

Berikut adalah katalog lengkap dan pasti dari seluruh konstanta publik, enum, set state, kode error, dan objek opsi konfigurasi default yang diekspor oleh `leaves-guardian`.
...
### 4.2 Normalisasi & Abstraksi Pesan
...
export const DEFAULT_RATE_LIMITER_OPTIONS = Object.freeze({
  windowMs: 60000,          // 60 detik
  maxRequests: 60,          // 60 request per window
  penaltyDurationMs: 0,     // 0 = Nonaktif
  maxTrackedKeys: 10000,    // Maksimal 10.000 key terpantau
  cleanupIntervalMs: 60000, // Interval pembersihan 60 detik
...
export const DEFAULT_DEDUPLICATOR_OPTIONS = Object.freeze({
  ttlMs: 300000,              // 5 menit fixed TTL
  maxTrackedMessages: 10000,  // Maksimal 10.000 pesan terpantau
  cleanupIntervalMs: 60000,   // Interval pembersihan 60 detik
...
```js
export const DEFAULT_EPHEMERAL_DURATION = 604800; // 7 Hari
```
